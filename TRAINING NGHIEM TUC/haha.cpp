...
void solve(){
    ...
    for (int i = 1; i <= n; i++){
        cin >> s[i];
        int hs = 0;
        for (auto x : s[i]) hs = (hs * base + x);
        hp[s[i].size()][hs] = 1;
        uniquelengths.insert(s[i].size());
    }
    string t;
    cin >> t;
    t = "!" + t;
    ...
    for (int i = 1; i < t.size(); i++){
        pw[i] = pw[i - 1] * base;
        hst[i] = (hst[i - 1] * base + t[i]);
    }
    auto geths = [](int x, int y) -> int{
        return hst[y] - hst[x - 1] * pw[y - x + 1];
    };
    dp[0] = 1;
    for (int i = 1; i < t.size(); i++){
        for (auto x : uniquelengths){
            if (x > i) break;
            int f = geths(i - x + 1, i);
            if (hp[x].count(f)) dp[i] = (dp[i] + dp[i - x]) % mod;
        }
    }
    cout << dp[t.size()-1];
}
