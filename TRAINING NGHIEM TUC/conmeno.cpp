...
string chuyen(__int128 r){
    string res = "";
    if (r == -1) return "-1";
    if (r == 0) return "0";
    while (r > 0){
        res = char(r%10+48)+res;
        r/=10;
    }
    return res;
}
...
void solve(){
    ...
    int g = 10000;
    for (int i = 1; i <= 1023; i++) xs[i] = 0;
    int invg = power(g,mod-2,mod);
    xs[0] = 1;
    int ans = 0;
    for (int i = 1; i <= n; i++){
        for (int u = 0; u < 1024; u++) cc[u] = xs[u];
        for (int u = 0; u < 1024; u++){
            int h = u ^ a[i];
            int ep = p[i] * invg % mod;
            int vl = (g - p[i])*invg%mod;
            xs[u] = (cc[h]*ep + vl*cc[u])%mod;
        }
    }
    for (int i = 1; i <= 1023; i++) ans = (ans + xs[i]*i%mod*i%mod)%mod;
    cout << ans << "\n";
}
signed main()
{
