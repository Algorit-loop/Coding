...
void solve(){
    cin >> n >> q;
    vector <ll> a(2*n + 1,0) , pre(2*n + 1,0);
    for (int i = 1; i <= n; i++) cin >> a[i];
    for (int i = n + 1; i <= 2*n; i++) a[i] = a[i - n];
    for (int i = 1; i <= 2*n; i++) pre[i] = pre[i - 1] + a[i];
    while (q--){
        ll l , r; cin >> l >> r;
        ll p1 = l / n - (l%n == 0) , p2 = r / n - (r%n == 0);
        // cout << "p1 = " << p1 << " p2 = " << p2 << "\n";
        ll res = (p2 - p1) * pre[n];
        res = res + pre[p1+n] - pre[p1+(l-1)%n+1 - 1];
        //cout << "l = " << (l-1)%n+1 << " r = " << (r-1)%n + 1 << "\n";
        // cout << " + " << pre[p1+n] - pre[p1+(l-1)%n+1] << "\n";
        // cout << " - " << (pre[p2+n] - pre[p2+(r-1)%n+1]) << "\n";
        res = res - (pre[p2+n] - pre[p2+(r-1)%n+1]);
        // res = res + a[p2+(r-1)%n+1];
    cout << res << "\n";
    }
}
int main()
